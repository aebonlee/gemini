...
const RULES_BASH = [
  ...
  { type: 'builtin', re: /\b(echo|cd|ls|cat|grep|sed|awk|find|mkdir|rm|cp|mv|chmod|chown|curl|wget|pip|npm|npx|node|python|python3|git|docker|sudo|apt|brew|yarn|pnpm|bun|gcloud)\b/g },
...
function tokenizeLine(line, rules) {
  ...
  for (const t of filtered) {
    if (t.start > pos) {
      result += escapeHtml(line.slice(pos, t.start));
    }
